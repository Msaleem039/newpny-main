...
const Eflayer = () => {
	...
	const handleSearch = (e) => {
		...
		const filtered = eflyers.filter(
			(eflyer) => eflyer.name.toLowerCase().includes(term) || eflyer.email.toLowerCase().includes(term)
		);
	...
	const handleDelete = async (id) => {
		try {
			...
			setEflyers(eflyers.filter((eflyer) => eflyer.id !== id)); // Update the state after deletion
			setFilteredEflyers(filteredEflyers.filter((eflyer) => eflyer.id !== id)); // Update filtered list
		} catch (error) {
			...
			{!isAddInstructorPage && (
				<>
					<div className='flex justify-between items-center mb-6'>
						<h2 className='text-2xl font-semibold text-gray-100 cursor-pointer'>E-Flyers</h2>
							...
							<tbody className='divide-y divide-gray-700'>
								{filteredEflyers.map((eflyer, index) => (
									...
									>
										...
										<td className='px-6 py-4 whitespace-nowrap'>{eflyer.category.Category_Name}</td>
										<td className='px-6 py-4 whitespace-nowrap'>{eflyer.course.course_Name}</td>
										<td className='px-6 py-4 whitespace-nowrap'>
											<span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${eflyer.status === 'Active' ? 'bg-green-800 text-green-100' : 'bg-red-800 text-red-100'}`}>
												{eflyer.status}
											</span>
										</td>
										<td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
											<button className='text-indigo-400 hover:text-indigo-300 mr-2'>Edit</button>
											<button className='text-red-400 hover:text-red-300' onClick={() => handleDelete(eflyer.id)}>
												Delete
											</button>
										</td>
									</motion.tr>
