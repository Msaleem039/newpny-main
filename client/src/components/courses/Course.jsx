              ...
              <tbody className='divide-y divide-gray-700'>
                {filteredCourses.length > 0 ? (
                  filteredCourses.map((course) => (
                    <motion.tr
                      key={course._id} // Ensure to use the correct ID here
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                    ...
                    >
                        ...
                        <div className='flex items-center'>
                          <div className='flex-shrink-0 h-10 w-10'>
                            <div className='h-10 w-10 rounded-full bg-gradient-to-r from-purple-400 to-blue-500 flex items-center justify-center text-white font-semibold'>
                              {course.course_Name ? course.course_Name.charAt(0) : "?"}
                            </div>
                          </div>
                          <div className='ml-4'>
                      ...
                      <td className='px-6 py-4 whitespace-nowrap'>
                        <span className='px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-800 text-blue-100'>
                          <img src={course.course_Image || "N/A"} alt="Course" />
                        </span>
                      </td>
                      <td className='px-6 py-4 whitespace-nowrap'>
                      ...
                      {/* <Link to={`/editcourse/${course._id}`}> Updated Link */}
    <button className='text-indigo-400 hover:text-indigo-300 mr-2' onClick={() => handleEdit1(course._id)}>
      Edit
    </button>
  {/* </Link> */}
  {/* <button onClick={() => handleEdit1(course._id)}>Edit</button> */}
                        <button
                          className="text-red-400 hover:text-red-300"
                          onClick={() => handleDelete(course._id)} // Ensure to use _id or appropriate key here
                        >
                          Delete
                        </button>
                      </td>
